...
const api = {
    ...
    getMarketProfiles(limit, includeNoise, categoryFilter) {
        var mysql = this.getFridaDb();
        if (!mysql) return [];

        var sql = "SELECT item_id, category, raw_category_code, market_tier, base_price, min_listings, max_listings, " +
        ...
        if (categoryFilter) sql += " AND category = '" + _escape(categoryFilter) + "'";
        ...
        for (var i = 0; i < n; i++) {
            ...
            rows.push({
                item_id: api_MySQL_get_int(mysql, 0),
                cname: '',
                category: api_MySQL_Get_Str(mysql, 1),
                raw_category_code: api_MySQL_get_int(mysql, 2),
                market_tier: api_MySQL_Get_Str(mysql, 3),
                base_price: api_MySQL_get_int(mysql, 4) || 1000,
                min_listings: api_MySQL_get_int(mysql, 5) || 0,
                max_listings: api_MySQL_get_int(mysql, 6) || 1,
                min_total_quantity: api_MySQL_get_int(mysql, 7) || 0,
                max_total_quantity: api_MySQL_get_int(mysql, 8) || 1,
                preferred_stack_min: api_MySQL_get_int(mysql, 9) || 1,
                preferred_stack_max: api_MySQL_get_int(mysql, 10) || 1,
                volatility: parseFloat(api_MySQL_Get_Str(mysql, 11)) || 0.15,
                bot_trade_weight: parseFloat(api_MySQL_Get_Str(mysql, 12)) || 0,
                system_trade_weight: parseFloat(api_MySQL_Get_Str(mysql, 13)) || 0,
                rotation_weight: parseFloat(api_MySQL_Get_Str(mysql, 14)) || 0
            });
        }
        return rows;
    },
    ...
    init() {
        ...
        var initFridaTables = function () {
            ...
            var defaultConfigs = [
                ...
                ['max_restocks_per_cycle', '500', '每轮最大补货数'],
                ['restock_equipment_per_cycle', '300', '每轮装备补货数'],
                ['restock_material_per_cycle', '100', '每轮材料补货数'],
                ['restock_consumable_per_cycle', '100', '每轮消耗品补货数'],
