...
const AUCTION_DB = 'taiwan_cain_auction_gold';
...
function _pickWeightedZone(zones) {
    if (!zones || zones.length === 0) return null;
    var total = 0;
    for (var i = 0; i < zones.length; i++) {
        total += Math.max(1, zones[i].weight || 1);
    }
    var roll = Math.floor(Math.random() * total);
    var acc = 0;
    for (var z = 0; z < zones.length; z++) {
        acc += Math.max(1, zones[z].weight || 1);
        if (roll < acc) return zones[z];
    }
    return zones[0];
}
...
function _readUserPosition(user) {
    return {
        x: user.add(577532).readU16(),
        y: user.add(577534).readU16(),
        area: user.add(577468).readS32()
    };
}
...
const api = {
    ...
    getCainDb() {
        ...
        try {
            dbConnector.init('taiwan_cain', _defaultConnect.ip, _defaultConnect.port, _defaultConnect.userName, _defaultConnect.password);
            return dbConnector.get('taiwan_cain', _defaultConnect.ip, _defaultConnect.port);
        } catch (e2) {
            log(WARN, '[auction] taiwan_cain数据库连接暂不可用: ' + e2);
            return null;
        }
    },

    describeCharacInfoColumns(limit) {
        var mysql = this.getCainDb();
        if (!mysql) return [];
        var max = parseInt(limit, 10) || 80;
        var sql = "SELECT COLUMN_NAME, DATA_TYPE, ORDINAL_POSITION FROM INFORMATION_SCHEMA.COLUMNS " +
            "WHERE TABLE_SCHEMA='taiwan_cain' AND TABLE_NAME='charac_info' ORDER BY ORDINAL_POSITION LIMIT " + max;
        ...
        for (var i = 0; i < n; i++) {
            ...
            rows.push({
                name: api_MySQL_Get_Str(mysql, 0),
                type: api_MySQL_Get_Str(mysql, 1),
                pos: api_MySQL_get_int(mysql, 2)
            });
        }
        return rows;
    },

    getPresenceCharacterSeed(charac_no) {
        var mysql = this.getCainDb();
        if (!mysql || !charac_no) return null;
        var sql = "SELECT charac_no, charac_name, m_id, lev, job, grow_type, village, expert_job, " +
            "delete_flag, UNIX_TIMESTAMP(create_time), UNIX_TIMESTAMP(last_play_time) FROM charac_info WHERE charac_no=" + (parseInt(charac_no, 10) || 0) + " LIMIT 1";
        ...
        return {
            ...
            lev: api_MySQL_get_int(mysql, 3),
            job: api_MySQL_get_int(mysql, 4),
            grow_type: api_MySQL_get_int(mysql, 5),
            village: api_MySQL_get_int(mysql, 6),
            expert_job: api_MySQL_get_int(mysql, 7),
            delete_flag: api_MySQL_get_int(mysql, 8),
            create_time: api_MySQL_get_int(mysql, 9),
            last_play_time: api_MySQL_get_int(mysql, 10)
        };
    },

    getPresenceCharacterSeedByUid(uid) {
        var mysql = this.getCainDb();
        if (!mysql || !uid) return null;
        var sql = "SELECT charac_no, charac_name, m_id, lev, job, grow_type, village, expert_job, " +
            "delete_flag, UNIX_TIMESTAMP(create_time), UNIX_TIMESTAMP(last_play_time) FROM charac_info " +
            "WHERE m_id=" + (parseInt(uid, 10) || 0) + " ORDER BY charac_no LIMIT 1";
        if (!api_MySQL_Exec_Safe(mysql, sql)) return null;
        if (MySQL_Get_N_Rows(mysql) == 0 || MySQL_Fetch(mysql) != 1) return null;
        return {
            charac_no: api_MySQL_get_int(mysql, 0),
    ...
    getActiveBotCharacters(role) {
        ...
        var sql = "SELECT charac_no, charac_name, m_id, role, is_active, min_price_margin, max_price_margin, max_listings_per_day, listings_today, last_reset_date, persona_type, tags, activity_level, projection_enabled, village, area, x, y FROM auction_bot_characters WHERE is_active = 1";
        if (role && role !== 'both') {
        ...
        for (var i = 0; i < n; i++) {
            ...
            rows.push({
                ...
                projection_enabled: api_MySQL_get_int(mysql, 13) || 0,
                village: api_MySQL_get_int(mysql, 14) || 0,
                area: api_MySQL_get_int(mysql, 15) || 0,
                x: api_MySQL_get_int(mysql, 16) || 0,
                y: api_MySQL_get_int(mysql, 17) || 0
            });
        }
        return rows;
    },
    ...
    /** 获取可投影到城镇的假人角色 */
    getProjectedBotCharacters(limit) {
        var mysql = this.getFridaDb();
        if (!mysql) return [];

        var sql = "SELECT b.charac_no, b.charac_name, b.m_id, b.role, b.persona_type, b.tags, b.activity_level, b.village, b.area, b.x, b.y, COALESCE(ci.lev, 1), COALESCE(ci.job, 0), COALESCE(ci.grow_type, 0) " +
            "FROM auction_bot_characters b LEFT JOIN taiwan_cain.charac_info ci ON ci.charac_no = b.charac_no " +
            "WHERE b.is_active = 1 AND b.projection_enabled = 1 ORDER BY b.charac_no";
        if (limit) sql += " LIMIT " + parseInt(limit, 10);
        if (!api_MySQL_Exec_Safe(mysql, sql)) return [];
        ...
        for (var i = 0; i < n; i++) {
            ...
            rows.push({
                ...
                m_id: api_MySQL_get_int(mysql, 2),
                role: api_MySQL_Get_Str(mysql, 3) || 'seller',
                persona_type: api_MySQL_Get_Str(mysql, 4) || 'material_merchant',
                tags: api_MySQL_Get_Str(mysql, 5) || '',
                activity_level: parseFloat(api_MySQL_Get_Str(mysql, 6)) || 1.0,
                village: api_MySQL_get_int(mysql, 7) || 0,
                area: api_MySQL_get_int(mysql, 8) || 0,
                x: api_MySQL_get_int(mysql, 9) || 0,
                y: api_MySQL_get_int(mysql, 10) || 0,
                lev: api_MySQL_get_int(mysql, 11) || 1,
                job: api_MySQL_get_int(mysql, 12) || 0,
                grow_type: api_MySQL_get_int(mysql, 13) || 0
            });
        }
        return rows;
    },
    ...
    /** 根据账号UID查找角色列表 */
    getCharactersByUid(uid) {
        var mysql = dbConnector.getConFromConfig('taiwan_cain');
        if (!mysql) return [];

        var id = parseInt(uid, 10) || 0;
        if (!id) return [];

        var sql = "SELECT charac_no, charac_name, m_id, lev, job, grow_type FROM charac_info WHERE m_id=" + id + " ORDER BY charac_no";
        if (!api_MySQL_Exec_Safe(mysql, sql)) return [];
    ...
    /** 查询在线角色指针和基础状态，只读 */
    getOnlineCharacterInfo(charac_no) {
        var id = parseInt(charac_no, 10) || 0;
        if (!id) return null;

        var user = _isCharacOnline(id);
        if (!user || user.isNull()) return null;

        var info = {
            charac_no: id,
            user_ptr: user.toString(),
            state: CUser_get_state(user),
            pos: _readUserPosition(user)
        };
        return info;
    },
    ...
    /** 从 d_taiwan.accounts.isRobot=1 批量导入假人角色 */
    importRobotAccounts(limit, role) {
        var cain = dbConnector.getConFromConfig('taiwan_cain');
        var fridaDb = this.getFridaDb();
        if (!cain || !fridaDb) return { scanned: 0, imported: 0 };

        var max = parseInt(limit, 10) || 500;
        var targetRole = role || 'seller';
        var sql = "SELECT ci.charac_no, ci.charac_name, ci.m_id, ci.lev " +
            "FROM taiwan_cain.charac_info ci INNER JOIN d_taiwan.accounts a ON a.UID = ci.m_id " +
            "WHERE a.isRobot = 1 ORDER BY ci.charac_no LIMIT " + max;
        if (!api_MySQL_Exec_Safe(cain, sql)) return { scanned: 0, imported: 0 };

        var rows = [];
        var n = MySQL_Get_N_Rows(cain);
        for (var i = 0; i < n; i++) {
            if (MySQL_Fetch(cain) != 1) break;
            rows.push({
                charac_no: api_MySQL_get_int(cain, 0),
                charac_name: api_MySQL_Get_Str(cain, 1),
                m_id: api_MySQL_get_int(cain, 2),
                lev: api_MySQL_get_int(cain, 3) || 1
            });
        }
        ...
        for (var r = 0; r < rows.length; r++) {
            var ch = rows[r];
            if (!ch.charac_no || !ch.m_id) continue;
            var insert = "INSERT INTO auction_bot_characters (charac_no, charac_name, m_id, role, is_active, min_price_margin, max_price_margin, max_listings_per_day, listings_today, projection_enabled) VALUES (" +
                ch.charac_no + ", '" + _escape(ch.charac_name || '') + "', " + ch.m_id + ", '" + _escape(targetRole) + "', 1, 1.1, 1.8, 5, 0, 0) " +
                "ON DUPLICATE KEY UPDATE charac_name = VALUES(charac_name), m_id = VALUES(m_id), is_active = 1";
            if (api_MySQL_exec_raw(fridaDb, insert) == 1) {
                this.ensureBotWallet(ch.charac_no);
                imported++;
            }
        }
        return { scanned: rows.length, imported: imported };
    },
    ...
    /** 获取启用的站街区域 */
    getSpawnZones(enabledOnly) {
        var mysql = this.getFridaDb();
        if (!mysql) return [];

        var sql = "SELECT id, village, area, name, min_level, x_min, x_max, y_min, y_max, weight, enabled FROM auction_dummy_spawn_zone";
        if (enabledOnly !== false) sql += " WHERE enabled = 1";
        sql += " ORDER BY village, area";
        if (!api_MySQL_Exec_Safe(mysql, sql)) return [];
        ...
        for (var i = 0; i < n; i++) {
            if (MySQL_Fetch(mysql) != 1) break;
            rows.push({
                id: api_MySQL_get_int(mysql, 0),
                village: api_MySQL_get_int(mysql, 1) || 0,
                area: api_MySQL_get_int(mysql, 2) || 0,
                name: api_MySQL_Get_Str(mysql, 3) || '',
                min_level: api_MySQL_get_int(mysql, 4) || 0,
                x_min: api_MySQL_get_int(mysql, 5) || 0,
                x_max: api_MySQL_get_int(mysql, 6) || 0,
                y_min: api_MySQL_get_int(mysql, 7) || 0,
                y_max: api_MySQL_get_int(mysql, 8) || 0,
                weight: api_MySQL_get_int(mysql, 9) || 100,
                enabled: api_MySQL_get_int(mysql, 10) || 0
            });
        }
        return rows;
    },
    ...
    getSpawnZoneCount() {
        var mysql = this.getFridaDb();
        if (!mysql) return { total: 0, enabled: 0 };
        if (!api_MySQL_Exec_Safe(mysql, "SELECT COUNT(*), SUM(CASE WHEN enabled = 1 THEN 1 ELSE 0 END) FROM auction_dummy_spawn_zone"))
            return { total: 0, enabled: 0 };
        if (MySQL_Get_N_Rows(mysql) == 0 || MySQL_Fetch(mysql) != 1) return { total: 0, enabled: 0 };
        return {
            total: api_MySQL_get_int(mysql, 0) || 0,
            enabled: api_MySQL_get_int(mysql, 1) || 0
        };
    },

    /** 给已导入假人分配站街位置 */
    assignProjectedBotLocations(limit) {
        var fridaDb = this.getFridaDb();
        if (!fridaDb) return { assigned: 0, candidates: 0, zones: 0 };

        var max = parseInt(limit, 10) || 500;
        var zones = this.getSpawnZones(true);
        if (zones.length === 0) return { assigned: 0, candidates: 0, zones: 0 };

        var sql = "SELECT b.charac_no, b.charac_name, b.m_id, COALESCE(ci.lev, 1) " +
            "FROM auction_bot_characters b LEFT JOIN taiwan_cain.charac_info ci ON ci.charac_no = b.charac_no " +
            "WHERE b.is_active = 1 ORDER BY RAND() LIMIT " + max;
        if (!api_MySQL_Exec_Safe(fridaDb, sql)) return { assigned: 0, candidates: 0, zones: zones.length };
        ...
        for (var i = 0; i < n; i++) {
            ...
            chars.push({
                charac_no: api_MySQL_get_int(fridaDb, 0),
                charac_name: api_MySQL_Get_Str(fridaDb, 1),
                m_id: api_MySQL_get_int(fridaDb, 2),
                lev: api_MySQL_get_int(fridaDb, 3) || 1
            });
        }
        ...
        for (var c = 0; c < chars.length; c++) {
            ...
            var eligible = [];
            for (var z = 0; z < zones.length; z++) {
                if (ch.lev >= zones[z].min_level) eligible.push(zones[z]);
            }
            if (eligible.length === 0) eligible = zones;

            var zone = _pickWeightedZone(eligible);
            if (!zone) continue;
            var xMin = Math.min(zone.x_min, zone.x_max);
            var xMax = Math.max(zone.x_min, zone.x_max);
            var yMin = Math.min(zone.y_min, zone.y_max);
            var yMax = Math.max(zone.y_min, zone.y_max);
            var x = xMin + Math.floor(Math.random() * Math.max(1, xMax - xMin + 1));
            var y = yMin + Math.floor(Math.random() * Math.max(1, yMax - yMin + 1));

            var update = "UPDATE auction_bot_characters SET projection_enabled = 1, village = " + zone.village +
                ", area = " + zone.area + ", x = " + x + ", y = " + y +
                " WHERE charac_no = " + ch.charac_no;
            if (api_MySQL_exec_raw(fridaDb, update) == 1) assigned++;
        }
        return { assigned: assigned, candidates: chars.length, zones: zones.length };
    },
    ...
    scanAuctionListings(itemId) {
        ...
        var sql = "SELECT auction_id, owner_id, owner_name, owner_type, item_id, unit_price, instant_price, add_info, upgrade, endurance, seal_flag, expire_time, occ_time FROM auction_main WHERE expire_time > UNIX_TIMESTAMP()";
        if (itemId) {
    ...
    getAuctionStats(itemId) {
        ...
        var sql = "SELECT COUNT(*), COALESCE(AVG(unit_price), 0), COALESCE(MIN(unit_price), 0), COALESCE(MAX(unit_price), 0), COALESCE(SUM(add_info), 0) FROM auction_main WHERE item_id = " + itemId + " AND expire_time > UNIX_TIMESTAMP()";
    ...
    listAuctionItem(listing) {
        ...
        var randomOptionSql = listing.random_option_sql || "UNHEX('')";
        var itemGuidSql = listing.item_guid_sql || "UNHEX('')";
        ...
        var sql = "INSERT INTO auction_main (" +
            ...
            listing.owner_id + ", '" + _escape((listing.owner_name || '系统').substring(0, 20)) + "', " + (listing.owner_type != null ? listing.owner_type : 1) + ", '" + _escape((listing.owner_nexon_id || '0').substring(0, 25)) + "', " +
            "-1, '', -1, " + instant_price + ", " +
            (listing.seal_flag != null ? listing.seal_flag : 0) + ", " + listing.item_id + ", " + (listing.add_info || 1) + ", " + (listing.upgrade || 0) + ", " +
            "0, 0, 0, " + (listing.endurance != null ? listing.endurance : 0) + ", " +
            "0, 0, " + listing.unit_price + ", " +
            randomOptionSql + ", 0, 0, 0, " + itemGuidSql +
            ")";
...
module.exports = {
    init() {
        ...
        var initFridaTables = function () {
            ...
            api_MySQL_exec_raw(fridaDb, "ALTER TABLE auction_bot_characters ADD COLUMN projection_enabled tinyint DEFAULT 0");
            api_MySQL_exec_raw(fridaDb, "ALTER TABLE auction_bot_characters ADD COLUMN village int DEFAULT 0");
            api_MySQL_exec_raw(fridaDb, "ALTER TABLE auction_bot_characters ADD COLUMN area int DEFAULT 0");
            api_MySQL_exec_raw(fridaDb, "ALTER TABLE auction_bot_characters ADD COLUMN x int DEFAULT 0");
            api_MySQL_exec_raw(fridaDb, "ALTER TABLE auction_bot_characters ADD COLUMN y int DEFAULT 0");

            // 假人站街区域表。可由 tools/import-dummy-map.ps1 从 dummy 的 地图.json 导入。
            api_MySQL_exec_raw(fridaDb,
                "CREATE TABLE IF NOT EXISTS auction_dummy_spawn_zone (" +
                "id int AUTO_INCREMENT PRIMARY KEY," +
                ...
                "enabled tinyint NOT NULL DEFAULT 1," +
                "source varchar(32) DEFAULT 'dummy_map'," +
                "updated_at datetime DEFAULT NULL," +
                "UNIQUE KEY uniq_zone (village, area)," +
                "KEY idx_enabled_level (enabled, min_level)" +
                ") ENGINE=InnoDB DEFAULT CHARSET=utf8");
