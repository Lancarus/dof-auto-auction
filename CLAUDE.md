...
**Database**: Uses `taiwan_cain_auction_gold.auction_main` (game auction table) and `frida.*` tables (auction_whitelist, auction_system_config, auction_bot_characters, auction_bot_config, auction_bot_log, auction_price_history, pending_mail). All tables auto-created on init.
