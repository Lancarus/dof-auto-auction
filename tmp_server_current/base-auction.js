...
// 默认连接参数（从现有配置推断）
const _defaultConnect = { ip: '127.0.0.1', port: 3306, userName: 'game', password: 'uu5!^%jg' };

////////////////////////////////////////////////////////////////////////
// 变量
////////////////////////////////////////////////////////////////////////

var _fridaCharsetReady = false;
...
function _ensureFridaCharset(mysql) {
    if (!mysql || _fridaCharsetReady) return;
    // The bundled MySQL client defaults to latin1, which turns UTF-8 names into '?' before JS sees them.
    api_MySQL_exec_raw(mysql, "SET NAMES utf8");
    _fridaCharsetReady = true;
}
...
const api = {
    ...
    getFridaDb() {
        ...
        try {
            ...
            var fallback = dbConnector.get(FRIDA_DB, _defaultConnect.ip, _defaultConnect.port);
            _ensureFridaCharset(fallback);
            return fallback;
        } catch (e2) {
    ...
    getMarketProfiles(limit, includeNoise) {
        ...
        var sql = "SELECT item_id, COALESCE(cname_display, cname), category, raw_category_code, market_tier, base_price, min_listings, max_listings, " +
    ...
    scanAuctionListings(itemId) {
        ...
        var sql = "SELECT auction_id, owner_id, owner_name, owner_type, item_id, unit_price, instant_price, add_info, upgrade, endurance, seal_flag, expire_time, occ_time FROM auction_main WHERE expire_time > " + api_CSystemTime_getCurSec();
        if (itemId) {
    ...
    getAuctionStats(itemId) {
        ...
        var sql = "SELECT COUNT(*), COALESCE(AVG(unit_price), 0), COALESCE(MIN(unit_price), 0), COALESCE(MAX(unit_price), 0), COALESCE(SUM(add_info), 0) FROM auction_main WHERE item_id = " + itemId + " AND expire_time > " + api_CSystemTime_getCurSec();
    ...
    init() {
        ...
        api_ScheduleOnMainThread(function () {
            ...
            api_MySQL_exec_raw(fridaDb,
                ...
                "item_id int unsigned NOT NULL PRIMARY KEY," +
                "cname varchar(64) DEFAULT NULL," +
                "cname_display varchar(64) DEFAULT NULL," +
                ...
                ") ENGINE=InnoDB DEFAULT CHARSET=utf8");
            api_MySQL_exec_raw(fridaDb, "ALTER TABLE auction_item_profile ADD COLUMN cname_display varchar(64) DEFAULT NULL");
